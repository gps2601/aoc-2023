...
fn criterion_benchmark_part1(c: &mut Criterion) {
    ...
    group.bench_with_input("part1", _input, |b, _input| {
        b.iter(part1::process)
    });
...
fn criterion_benchmark_part2(c: &mut Criterion) {
    let _input = include_str!("../input2.txt");

    let mut group = c.benchmark_group("day_06::part2");
    group.bench_with_input("part2", _input, |b, _input| {
        b.iter(part2::process)
    });
